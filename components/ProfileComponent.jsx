...
import { StyleSheet, View, Text,Pressable,FlatList,ImageBackground,SafeAreaView } from "react-native";
...
const ProfileComponent = () => {
    ...
    const fetchDataParticipate = async () => {
      ...
      for (const raffleDoc of rafflesSnapshot.docs) {
        ...
        ticketsSnapshot.forEach((doc) => {
          allTickets.push({ id: doc.id,title:raffleData.titulo, ...doc.data() });
        });
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <ImageBackground source={image} resizeMode="cover" imageStyle={{height:110}}>
      ...
      {raffles.map((doc) => (
        <View key={doc.id} style={styles.rafflesContainer}>
          <View style={styles.raffleTitleContainer}>
            <Text style={styles.rafflesCreated}>Has creado una rifa con el nombre de <Text style={styles.rafflesCreated__tile}>"{doc.titulo}"</Text></Text>
          </View>
